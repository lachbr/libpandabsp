...
string DistributedObjectAI::
get_delete_event() const
{
        stringstream ss;
...
int DistributedObjectAI::
get_delay_delete_count() const
{
        return 0;
}
...
void DistributedObjectAI::
add_interest( ZONEID_TYPE zone, string note, string event )
{
...
void DistributedObjectAI::
update_all_required_other_fields( DCClassPP *dclass, DatagramIterator &di )
{
        dclass->receive_update_all_required( this, di );
        announce_generate();
        post_generate_message();
...
string DistributedObjectAI::
get_zone_change_event() const
{
        return get_zone_change_event_static( _do_id );
}

string DistributedObjectAI::
get_logical_zone_change_event() const
{
        return get_logical_zone_change_event_static( _do_id );
}

string DistributedObjectAI::
get_zone_change_event_static( DOID_TYPE doid )
{
        stringstream ss;
...
string DistributedObjectAI::
get_logical_zone_change_event_static( DOID_TYPE doid )
{
        stringstream ss;
...
void DistributedObjectAI::
handle_logical_zone_change( ZONEID_TYPE new_zone, ZONEID_TYPE old_zone )
{
        throw_event( get_logical_zone_change_event(), EventParameter( ( int )new_zone ),
                     EventParameter( ( int )old_zone ) );
}

CHANNEL_TYPE DistributedObjectAI::
get_puppet_connection_channel( DOID_TYPE do_id )
{
        return do_id + ( ( CHANNEL_TYPE )1 << 32 );
}

CHANNEL_TYPE DistributedObjectAI::
get_account_connection_channel( DOID_TYPE do_id )
{
        return do_id + ( ( CHANNEL_TYPE )3 << 32 );
}

DOID_TYPE DistributedObjectAI::
get_account_id_from_channel_code( CHANNEL_TYPE chan )
{
        return chan >> 32;
}

DOID_TYPE DistributedObjectAI::
get_avatar_id_from_channel_code( CHANNEL_TYPE chan )
{
...
void DistributedObjectAI::
generate_with_required( ZONEID_TYPE zone )
{
        if ( _prealloc_do_id )
        {
                _prealloc_do_id = false;
...
void DistributedObjectAI::
generate_with_required_and_id( DOID_TYPE doid, DOID_TYPE parent, ZONEID_TYPE zone )
{
        if ( _prealloc_do_id )
        {
...
CHANNEL_TYPE DistributedObjectAI::
generate_target_channel() const
{
        return ( ( AIRepositoryBase * )g_air )->get_server_id();
}

void DistributedObjectAI::
send_generate_with_required( DOID_TYPE parent, ZONEID_TYPE zone )
{
