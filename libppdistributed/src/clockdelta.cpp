...
ClockDelta *ClockDelta::_global_ptr = ( ClockDelta * )NULL;
...
get_global_ptr()
{
        if ( _global_ptr == ( ClockDelta * )NULL )
        {
...
void ClockDelta::
resynchronize( double localtime, double nettime, double newuncertaincy, bool trustnew )
{
...
bool ClockDelta::
p2p_resync( uint32_t avid, int timestamp, double servertime, double uncertainty )
{
        double now = g_global_clock->get_real_time();
...
bool ClockDelta::
new_delta( double localtime, double newdelta, double newuncertainty, bool trust_new )
{
...
double ClockDelta::
network_to_local_time( int nettime, double now, int bits, double tickspersec )
{
        if ( now == -1 )
        {
...
int ClockDelta::
local_to_network_time( double localtime, int bits, double tickspersec )
{
...
int ClockDelta::
get_real_network_time( int bits, double tickspersec )
{
...
int ClockDelta::
get_frame_network_time( int bits, double tickspersec )
{
...
double ClockDelta::
local_elapsed_time( int nettime, int bits, double tickspersec )
{
...
void ClockDelta::
reset_clock( double timedelta )
{
        _delta += timedelta;
}

void ClockDelta::
reset_clock_event( const Event *e, void *data )
{
