...
DistributedObjectBase *DoCollectionManager::
get_do( DOID_TYPE doid )
{
...
int DoCollectionManager::
get_game_do_id() const
{
        return game_globals_id;
}

DistributedObjectBase *DoCollectionManager::
get_owner_view( DOID_TYPE doid )
{
        nassertr( dc_has_owner_view, NULL );
        return _do_id_2_owner_view[doid];
}

DoCollectionManager::DoMap &DoCollectionManager::
get_do_table( bool ownerview )
{
        if ( ownerview )
        {
...
do_find( const string &str )
{
        ...
        return ( DistributedObjectBase * )NULL;
}

vector<DistributedObjectBase *> DoCollectionManager::
do_find_all( const string &str )
{
...
bool DoCollectionManager::
has_owner_view_do_id( DOID_TYPE doid ) const
{
...
void DoCollectionManager::
handle_object_location( DatagramIterator &di )
{
...
void DoCollectionManager::
store_object_location( DistributedObjectBase *dobj, DOID_TYPE parent, ZONEID_TYPE zone )
{
...
void DoCollectionManager::
delete_object_location( DistributedObjectBase *dobj, DOID_TYPE parent, ZONEID_TYPE zone )
{
...
void DoCollectionManager::
add_do_to_tables( DistributedObjectBase *dobj, DOID_TYPE parent, ZONEID_TYPE zone, bool ownerview )
{
...
void DoCollectionManager::
remove_do_from_tables( DistributedObjectBase *dobj )
{
...
DoCollectionManager::DoMap DoCollectionManager::
get_objects_in_zone( DOID_TYPE parentid, ZONEID_TYPE zoneid )
{
        DoMap result;
        for ( DoMap::iterator itr = _do_id_2_do.begin(); itr != _do_id_2_do.end(); ++itr )
        {
...
DoCollectionManager::DoMap DoCollectionManager::
get_objects_of_class_in_zone( DOID_TYPE parentid, ZONEID_TYPE zoneid,
                              TypeHandle &handle )
{
        DoMap result;
        for ( DoMap::iterator itr = _do_id_2_do.begin(); itr != _do_id_2_do.end(); ++itr )
