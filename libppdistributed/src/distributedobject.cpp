...
bool DistributedObject::is_local() const
{
        return g_cr->is_local_id( _do_id );
}

void DistributedObject::b_set_location( DOID_TYPE parent, ZONEID_TYPE zone )
{
        d_set_location( parent, zone );
        set_location( parent, zone );
}

void DistributedObject::d_set_location( DOID_TYPE parent, ZONEID_TYPE zone )
{
}

void DistributedObject::set_location( DOID_TYPE parent, ZONEID_TYPE zone )
{
...
vector<ZONEID_TYPE> DistributedObject::get_auto_interests()
{
...
void DistributedObject::delete_or_delay()
{
        if ( 0 )
        {
...
void DistributedObject::disable_announce_and_delete()
{
...
string DistributedObject::get_disable_event() const
{
        return unique_name( "disable" );
}

void DistributedObject::disable_and_announce()
{
...
void DistributedObject::announce_generate()
{
}

void DistributedObject::deactivate_do()
{
        if ( g_cr == nullptr )
        {
                distributedObject_cat.warning()
                        << "cr is nullptr in deactivate_do " << _do_id << "\n";
                return;
        }
...
void DistributedObject::destroy_do()
{
        _dclass = nullptr;
...
void DistributedObject::disable()
{
}

bool DistributedObject::is_disabled() const
{
        return _active_state == ES_disabled;
}

bool DistributedObject::is_generated() const
{
        return _active_state == ES_generated;
}

void DistributedObject::delete_do()
{
}

void DistributedObject::generate()
{
        _active_state = ES_generating;
        g_cr->open_auto_interests( this );
}

void DistributedObject::generate_init()
{
        _active_state = ES_generating;
}

void DistributedObject::post_generate_message()
{
...
void DistributedObject::update_required_fields( DCClassPP *dclass, DatagramIterator &di )
{
...
void DistributedObject::update_all_required_fields( DCClassPP *dclass, DatagramIterator &di )
{
...
void DistributedObject::update_required_other_fields( DCClassPP *dclass, DatagramIterator &di )
{
        dclass->receive_update_broadcast_required( this, di );
        announce_generate();
        post_generate_message();
...
void DistributedObject::send_disable_msg()
{
        // unused apparently
}

void DistributedObject::send_delete_msg()
{
        // Unused apparently
}

void DistributedObject::add_interest( ZONEID_TYPE zone, const string &note, const string &event )
{
...
void DistributedObject::remove_interest( int handle, const string &event )
{
        g_cr->remove_interest( handle, event );
}

bool DistributedObject::is_grid_parent() const
{
        return false;
}