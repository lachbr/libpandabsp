...
void GPUCommandQueue::
register_input( const ShaderInput *inp )
{
...
int GPUCommandQueue::
get_num_processed_commands() const
{
        return _pta_num_commands[0];
}

size_t GPUCommandQueue::
get_num_queued_commands()
{
        return _list.get_num_commands();
}

GPUCommandList *GPUCommandQueue::
get_command_list()
{
        return &_list;
}
