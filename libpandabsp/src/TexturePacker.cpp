...
public:
                                        ...
                                        int max_wid, int max_hit )
        {
                ...
                // Pack it
                PackResult new_pack = tp->packTextures( forcePowerOfTwo, onePixelBorder );
                delete ( tp );

                return ( new_pack.get_width() <= max_wid && new_pack.get_height() <= max_hit );
        }
        ...
        virtual void addNewTexture( int wid, int hit )
        {
                Texture *oldtexs = new Texture[getTextureCount()];
                memcpy( oldtexs, mTextures, sizeof( Texture ) * getTextureCount() );

                setTextureCount( getTextureCount() + 1 );
                // Add the old textures
                for ( size_t idx = 0; idx < getTextureCount() - 1; idx++ )
                {
                        addTexture( oldtexs[idx].mWidth, oldtexs[idx].mHeight );
                }
        ...
        virtual PackResult packTextures( bool forcePowerOfTwo, bool onePixelBorder )  // pack the textures, the return code is the amount of wasted/unused area.
        {
                int width = 0;
                int height = 0;
                ...
                return PackResult( width, height, ( width*height ) - mTotalArea );
        }
        ...
        virtual TextureLocation  getTextureLocation( int index )
        {
                bool ret = false;

                int x = 0, y = 0, wid = 0, hit = 0;
                ...
                return TextureLocation( ret, x, y, wid, hit );
        }
...
TexturePacker * TexturePacker::createTexturePacker( void )
{
        MyTexturePacker *m = new MyTexturePacker;
        return static_cast< TexturePacker *>( m );
}

void            TexturePacker::releaseTexturePacker( TexturePacker *tp )
{
